...
app.get("/", function (request, response) {
  try {
    ...
    console.error("error getting root endpoint", error);
  }
});
...
WHERE players.id = $1 AND player_quests.completed = FALSE`,
    ...
    console.error("error getting quests player is on", error);
  }
});
...
app.get("/quests", async (request, response) => {
  try {
    ...
    console.error("error fetching quests list", error);
  }
});
...
WHERE NOT EXISTS (
    ...
    console.error("error fetching available quests for player", error);
  }
});
